...
def my_tokenizer(s):
    ...
    #only keep the words longer than 2 letters
...
# To build a feature Vector for each review, we essentially need three ingredients:
    # 1. input sentence (each review in this case)
    # 2. empty feature vector of all 0s of size V (here we use capital V to denote the total vocabulary size of reviews)
    # 3. a dictionary that maps word to index (we need to know which index in the feature vector represents which word)

# building ingredient 3, the dictionary
...
# since long reviews with many words are represented by long arrow vectors and short reviews are represented by short vectors,
# we don't want our weights obtained to be influenced by the length of vectors so we normalized all feature vectors
# another reason we want to normalize is that since the sigmoid function approaches 1 or 0 very quickly,(eg. you input 10 and 100
# will almost get the same result that is very close to 1) s o you don't want your input to vary a lot and some of them being
# very large.

# there are two parameters to this function, the tokens of a review and the positive or negative label
# later we will run this function on every positive reviews with the label parameter equal to 1 and
def tokens_to_vector(tokens,label):
    # we make the size of x as n+1 since we want to add label (our y)(pos or neg )to the end of each x
    x = np.zeros(n+1)
    for t in tokens:
        # in
        i = word_index_map[t]
...
# building ingredient 2, but here we combine all feature vectors for all reviews together as a matrix
...
# random shuffle randomly change the order of rows without effecting contents within each row
np.random.shuffle(matrix)

# assign the normalized counts feature vectors to X
# this line means assigning all the contents within each row except the last column of matrix to X
X = matrix[:,:-1]
# assign the pos or neg binary labels to Y
# this line means assigning only the last col of each row of matrix to Y
Y = matrix[:,-1]

# split X and Y to train and test data
Xtrain,Xtest,Ytrain,Ytest = train_test_split(X,Y,test_size=0.2)

print('Statistics of Logistic Regression model')
